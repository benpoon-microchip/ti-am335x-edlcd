	...
	*/
	...
	if (sdio->eflags & SDMMC_CAP_DMA) {
		...
		*/
		...
		//slogf(_SLOGC_NETWORK, _SLOG_ERROR,"Debug: get %d bytes. data0=0x%x, data1=0x%x, data2=0x%x, data3=0x%x\n", rdbytes, buffer[0], buffer[1], buffer[2], buffer[3]);
				...
				uint8_t opcode, uint32_t blkcnt, uint32_t blksz, uint8_t *buffer, off64_t paddr)
{
	...
	*/
	...
	{
		//printf("CMD53 read1...\r\n");
		//cmd.eflags	= SDMMC_CMD_INTR | SDMMC_CMD_DATA;
		//sleep(5);
