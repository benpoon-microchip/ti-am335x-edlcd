...
static int omap3_ienable(void *hdl, int enable)
{
...
static int omap3_iprocess(void *hdl, sdio_cmd_t *cmd)
{
	...
	uint32_t       ests, nsts;
	int				intr = 0;
	//DEBUG_CMD(slogf(_SLOGC_SIM_MMC, _SLOG_ERROR, "%s ise %x ie %x stat %x", __FUNCTION__,
	//in32(base + OMAP3_MMCHS_ISE), in32(base + OMAP3_MMCHS_IE), in32(base + OMAP3_MMCHS_STAT));)
	...
	if (nsts & INTR_CIRQ) {
		//printf("[omap3_iprocess] INTR_CIRQ\n");
	...
	if (ests) {
		//printf("[omap3_iprocess] ests = 0x%x\n", ests);
		slogf(_SLOGC_NETWORK, _SLOG_ERROR,"[%s] ests = 0x%x\n", __func__, ests);
		if (ests & INTR_DTO) {
		...
		slogf(_SLOGC_NETWORK, _SLOG_ERROR,"[%s] nsts = 0x%x\n", __func__, nsts);
		if (nsts & INTR_CC) {
	...
	//DEBUG_CMD(slogf(99,1,"%s intr %x", __FUNCTION__, intr);)
	return intr;
}
static int omap3_setup_pio(void *ext_hdl, char *buf,int len, int dir)
{
	...
	int				nblk;
	//printf("omap3_setup_pio\r\n");
	nblk = len / omap3->blksz;
	len  = nblk * omap3->blksz;

	omap3->dcmd = CMD_DP;
	if (nblk > 1) {
		//printf("omap3_setup_pio, nblk > 1\r\n");
	...
	//printf("omap3_setup_pio, nblk=%d, blksz=%d\r\n", nblk, omap3->blksz);
...
static int omap3_pio_done(void *ext_hdl, char *buf, int len, int dir)
{
	...
	//printf("[omap3_pio_done] cnt = %d, nbytes = %d, len = %d\r\n", cnt, nbytes, len);
	if (dir == MMC_DIR_IN) {
	...
	//printf("[omap3_pio_done] 2 cnt = %d, nbytes=%d, byte_sent = %d, len = %d\r\n", cnt, nbytes, byte_sent, len);
	//return cnt;
	return byte_sent;
}
...
static int omap3_command(void *ext, sdio_cmd_t *cmd)
{
	...
	///slogf (_SLOGC_SIM_MMC, _SLOG_ERROR, "[%s] In\n", __func__);
