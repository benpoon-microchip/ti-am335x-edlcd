...
{
	slogf(_SLOGC_NETWORK, _SLOG_ERROR,"[%s] In, size = %d pkt_offset = %d \n", __func__, size, pkt_offset);
...
static int pkt_rx_task(void *vp)
{
	...
	while (1) {
		...
		pthread_mutex_lock(&wl->rx_queue);
		...
		if (!list_empty(&wl->rx_q.list))
		{
			...
			pthread_mutex_unlock(&wl->rx_queue);
...
static int wilc_txq_task(void *vp)
{
	...
	while (1) {
		...
		slogf(_SLOGC_NETWORK, _SLOG_ERROR,"txq_task Taking a nap\n");
		...
		slogf(_SLOGC_NETWORK, _SLOG_ERROR,"txq_task Who waked me up\n");
		...
		slogf(_SLOGC_NETWORK, _SLOG_ERROR,"handle the tx packet\n");
		do {
			...
			if (txq_count < FLOW_CTRL_LOW_THRESHLD) {
				...
				slogf(_SLOGC_NETWORK, _SLOG_ERROR,"Waking up queue\n");
...
static int wilc_firmware_download(struct wilc_dev *wilc)
{
	...
	if (!wilc->fw_file) {
		...
		wilc->fw_file = fopen ("/etfs/usr/lib/wilc1000_wifi_firmware.bin", "r");
...
fail_wilc_wlan:
		...
		PRINT_INFO(vif->ndev, INIT_DBG, "wilc already initialized\n");
	}
	return ret;
}
...
int wilc_mac_open(struct wilc_vif *vif, unsigned char mac_add[])
{
	...
	PRINT_INFO(ndev, INIT_DBG, "MAC OPEN[]\n");
...
void wilc_netdev_cleanup(struct wilc_dev *wilc)
{
	...
	*/
	...
	PRINT_INFO(ndev, GENERIC_DBG, "Module_exit Done.\n");
}
